...
- **Tasks**: Build THAID, an offline-first voice assistant that combines Ollama (LLM), Whisper (STT), and Piper (TTS) behind an on-demand D-Bus Rust daemon with a QML/PyQt6 desktop UI.
- **Key Behaviors**: Use D-Bus session activation instead of a boot-time daemon, load models lazily to conserve RAM, persist conversation memory locally, and expose a shared API for both the floating Orb and other desktop apps.
- **User Experience**: Provide a click-to-talk assistant with listening, thinking, and speaking states, plus responsive audio feedback and fallback handling when the AI backend is unavailable.
